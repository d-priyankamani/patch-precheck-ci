			     ...
			     Dwarf_Die *die)
{
	...
	type_die_tag = dwarf_tag(&type_die);
	if (type_die_tag == DW_TAG_unspecified_type) {
		die_name = dwarf_diename(die);
		printf("Found unnamed type die from %s, type_die_tag:%d\n", die_name, type_die_tag);
		return obj_basetype_new(safe_strdup("void"));
	}
			    ...
			    Dwarf_Die *die)
{
	...
	default: {
		...
		fail("Unexpected tag for symbol %u %s: %s\n", tag, name, tagname);
		break;
	}
	}
...
const char *debuginfo_paths[] = {"/tmp/kabi/usr/lib/debug/lib/modules:"DEBUG_MODULE_DIR};
...
static void generate_type_info(char *filepath, struct file_ctx *ctx)
{
	static const Dwfl_Callbacks callbacks = {
		...
		.debuginfo_path = (char **)debuginfo_paths,
	};
