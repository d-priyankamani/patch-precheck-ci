 ...
 * ck_reserved_. This is not the most specific string. It currently
 ...
 * *ck_reserved_ptrsX" instead of the usual "unsigned long
 * ck_reservedX". Treat this case as an exception.
 *
...
static int hide_kabi_cb(obj_t *o, void *args)
{
	...
	if (o->name) {
		...
		if (!strncmp(o->name, "ck_reserved_", 12) &&
		    strncmp(o->name, "ck_reserved_ptrs", 16)) {
	...
	if (!kabi_struct->ptr || kabi_struct->ptr->type != __type_struct ||
		...
		fail("Unexpeted ck_kabi_hide struct format\n");

	/*
	 * It is a ck_kabi_hide union
	 ...
	 *       <var, "__UNIQUE_ID_ck_kabi_hide55", "(null)", 0x1ea9700 0 0 0>
